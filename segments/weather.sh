...
__yahoo_weather() {
	...
	if [ -z "$degree" ]; then
		weather_data=$(curl --max-time 4 -s "http://query.yahooapis.com/v1/public/yql?q=select+%2A+from+weather.forecast+where+woeid+%3D+${TMUX_POWERLINE_SEG_WEATHER_LOCATION}+and+u%3D%27${TMUX_POWERLINE_SEG_WEATHER_UNIT}%27&format=xml")
		if [ "$?" -eq "0" ]; then
